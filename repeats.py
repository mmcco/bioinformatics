...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print "error - usage: python [-O] repeats.py <data directory>"
        exit(1)
    dataDir = sys.argv[1]
    dataDir = dataDir[:-1] if dataDir[-1] == '/' else dataDir
    # find all reference genome files and generate their absolute paths
    chromFilenames = filter(lambda filename: filename.endswith(".fa"), os.listdir(dataDir))
    chromFilenames = [dataDir + '/' + filename for filename in chromFilenames]
    repeatFilename = dataDir + '/' + dataDir + ".fa.out"
